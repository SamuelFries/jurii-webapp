...
/**
 * O cartão que responde à mão: inclina alguns graus na direção do cursor
 * (perspectiva, não truque de sombra) e acende a borda no ponto mais
 * próximo dele, como metal pegando luz.
 *
 * Regras de casa:
 *  - `prefers-reduced-motion` desliga o tilt E o brilho que anda; o cartão
 *    fica parado com a borda serena.
...
export function CartaoVivo({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    function aoMover(evento: PointerEvent) {
      ...
      // O brilho da borda segue o cursor mesmo com foco num campo: luz
      // pode andar; o chão, não.
      cartao.style.setProperty("--mx", `${(relativoX * 100).toFixed(2)}%`);
      cartao.style.setProperty("--my", `${(relativoY * 100).toFixed(2)}%`);
      ...
      quadro = window.requestAnimationFrame(() => {
        ...
        const giroY = (dentroX - 0.5) * 4; // graus
        const giroX = (0.5 - dentroY) * 3;
